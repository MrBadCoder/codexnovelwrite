...
3. 允许调用 `Read/Grep/Bash` 与 Codex 子流程、普通交互提问、Codex Web 工具辅助收集。
...
- Codex 子流程：拆分并行子任务（如题材映射、约束包候选生成、文件验证）。
- 普通交互提问：用于关键分歧裁决、候选方案选择、最终确认。
- Codex Web 工具（search/open）：用于检索市场趋势、平台风向、题材数据并做事实核验。
- 外部检索触发条件：
  - 用户明确要求参考市场趋势或平台风向；
  - 创意约束需要“时间敏感依据”；
  - 对题材信息存在明显不确定。
