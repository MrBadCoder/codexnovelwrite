...
from data_modules.runtime_paths import get_user_config_dir, get_user_config_dirs


DEFAULT_PROJECT_DIR_NAMES: tuple[str, ...] = ("webnovel-project",)
CURRENT_PROJECT_POINTER_RELS: tuple[Path, ...] = (
    Path(".codex") / ".webnovel-current-project",
    Path(".claude") / ".webnovel-current-project",
)
GLOBAL_REGISTRY_FILE = "workspaces.json"
...
def _global_registry_paths_read() -> list[Path]:
    return [path / GLOBAL_REGISTRY_FILE for path in get_user_config_dirs()]


def _global_registry_path_write() -> Path:
    return get_user_config_dir() / GLOBAL_REGISTRY_FILE
...
def _resolve_project_root_from_global_registry(
    ...
    *,
    ...
    for env_name in (ENV_WEBNOVEL_WORKSPACE_ROOT, ENV_CLAUDE_PROJECT_DIR):
        env_ws = os.environ.get(env_name)
        if env_ws:
    ...
    for reg_path in _global_registry_paths_read():
...
def update_global_registry_current_project(
    *,
    ...
    reg_path = _global_registry_path_write()
    reg = _load_global_registry(reg_path)
...
def _resolve_project_root_from_pointer(cwd: Path, *, stop_at: Optional[Path] = None) -> Optional[Path]:
    ...
    - relative path is also supported (resolved relative to pointer marker dir).
    """
...
def _find_workspace_root_with_runtime_marker(start: Path) -> Optional[Path]:
    """Find nearest ancestor containing `.codex/` or `.claude/` marker."""
...
def _pick_workspace_pointer_file(ws_root: Path) -> Optional[Path]:
    for rel in CURRENT_PROJECT_POINTER_RELS:
        if (ws_root / rel.parent.name).is_dir():
            return ws_root / rel
    return None
...
def write_current_project_pointer(project_root: Path, *, workspace_root: Optional[Path] = None) -> Optional[Path]:
    ...
    ws_root = (
        ...
        if workspace_root
        else _find_workspace_root_with_runtime_marker(root)
    )
    if ws_root is None:
        ws_root = _find_workspace_root_with_runtime_marker(Path.cwd().resolve())
    if ws_root is None:
        # 兜底：若无法找到 `.codex/` 或 `.claude/`，将项目父目录视为“工作区”候选，
        # 仅用于写入用户级 registry（不创建运行时目录，不写 pointer 文件）。
    ...
    if ws_root is not None:
        # 仅在工作区已存在运行时目录时写入指针，优先 `.codex/`，兼容 `.claude/`。
        selected = _pick_workspace_pointer_file(ws_root)
        if selected is not None:
            try:
                pointer_file = selected
...
def resolve_project_root(explicit_project_root: Optional[str] = None, *, cwd: Optional[Path] = None) -> Path:
    ...
    if explicit_project_root:
        ...
        # 兼容：显式传入“工作区根目录”但其运行时目录在用户目录（全局安装）时，
